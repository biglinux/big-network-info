...
class PDFExporter:
    ...
    def export_to_pdf(
        ...
        if filename is None:
            ...
            filename = _("network_scan_report_{timestamp}.pdf").format(
                timestamp=timestamp
            )
    ...
    def _add_header(
        ...
        subtitle_text = _("Generated on {time}").format(time=scan_time)
        if results is not None:  # Specific to scan report
            host_count = len(results)
            service_count = sum(len(result.services) for result in results)
            subtitle_text += "<br/>" + _(
                "<b>{host_count}</b> devices discovered with <b>{service_count}</b> total services"
            ).format(host_count=host_count, service_count=service_count)
    ...
    def _add_host_category(
        ...
        story.append(
            Paragraph(
                _("{title} ({count} devices)").format(title=title, count=len(hosts)),
    ...
    def _add_footer(self, story: list):
        ...
        footer_text = _(
            "📋 Report generated by <b>Big Network Info</b> • {date_time}"
    ...
    def export_diagnostics_to_pdf(
        ...
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = _("network_diagnostics_report_{timestamp}.pdf").format(
                timestamp=timestamp
            )
    ...
    def _add_diagnostics_recommendations(
        ...
        for i, step in enumerate(failed_or_warning_steps, 1):
            ...
            if step.troubleshooting_tip:
                ...
                recommendation += _("Review details: {details}").format(
